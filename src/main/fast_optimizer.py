...
def save_best_config(config):
    """Save best configuration to output/configs/best_config.txt (iter 3 - unified output dir)."""
    import os
    os.makedirs(os.path.join('output', 'configs'), exist_ok=True)
    with open('output/configs/best_config.txt', 'w') as f:
        f.write("# Best Scalping Strategy Configuration\n")
        f.write("# Generated by fast_optimizer.py\n\n")
        for key, value in config.items():
            f.write(f"{key}={value}\n")
    print(f"Best configuration saved to output/configs/best_config.txt")
