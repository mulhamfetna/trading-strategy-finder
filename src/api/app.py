...
_DATA_SUBDIRS = ('full_data', '2025_data', '2026_data')


@app.get("/api/data-files")
def list_data_files() -> dict:
    """Return CSV files grouped by dataset subdirectory under `data/`.

    Response shape:
        {
          "files": [<flat list of repo-relative paths>],
          "groups": {
            "full_data":  [<repo-relative paths>],
            "2025_data":  [<repo-relative paths>],
            "2026_data":  [<repo-relative paths>],
          }
        }

    The flat `files` list is preserved for back-compat with the existing
    FilePicker; new callers should use `groups`.
    """
    ...
    for sub in _DATA_SUBDIRS:
        ...
        rels = sorted(
            ...
            if f.lower().endswith('.csv') and os.path.isfile(os.path.join(sub_path, f))
        )
        groups[sub] = rels
        flat.extend(rels)
    return {"files": flat, "groups": groups}
...
async def upload_data_file(file: UploadFile = File(...)) -> dict:
    ...
    root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    # Uploads land in data/full_data/ — the canonical "manual selection"
    # store. Per-year splits are generated by scripts/split_data_by_year.py.
    upload_dir = os.path.join(root, 'data', 'full_data')
