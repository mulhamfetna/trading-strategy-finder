...
21 columns in fixed order. See [[signals-stage2-master]] §3 for the high-level spec; this card is the per-column quick reference.
...
| 14 | `last_signal`      | enum  | `long` or `short` — always the opposite of `first_signal` |
...
Examples (from `reverse_signals_full.csv`):
...
| Preset | Rows | long→short | short→long |
|---|---|---|---|
| `full` | 372 | 186 | 186 |
| `2025` | 253 | 127 | 126 |
| `2026` | 118 | 59 | 59 |

The `full` figures are regression-locked in [[generate_stage2_real_data]]. Per-year preset figures are derivatives.

## Ordering guarantee

Rows are sorted by `first_datetime` ASC. Because adjacent windows share endpoints, `df.last_datetime.iloc[i] == df.first_datetime.iloc[i+1]` for most adjacent pairs (exceptions: where a same-state discard intervened between two emitted windows).
