...
description: subprojects/signals/stage1_0_reverse_signals/dashboard/index.html — interactive X/Y scatter viewer for reverse_signals_full.csv
type: file
---
...
- `subprojects/signals/stage1_0_reverse_signals/dashboard/index.html` — the only file. Inline CSS + JS, no build step, no other assets.
...
The server **must root at `subprojects/signals/stage1_0_reverse_signals/`**, not inside `dashboard/`. `python3 -m http.server` refuses to serve files outside its root, and the page fetches `../reverse_signals_full.csv` (one level up from itself).
...
The project-wide `.gitignore` ignores `*.html` (output artifacts). The dashboard is whitelisted via `!subprojects/signals/stage1_0_reverse_signals/dashboard/*.html`. Adding more HTML files to this directory is the supported extension path; HTML elsewhere remains ignored.
