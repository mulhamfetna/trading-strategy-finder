...
def validate_params(params):
    ...
    # Max-hold TIME_CAP (decision #verbose_logs_audit): cap_1min = max traded 1-min bars a trade may
    # stay open (0 = off); cap_mode = none|bars|eod. Forwarded to the exact engine so the bundle honors
    # the wsh6cold 448-bar cap (the canonical fast-engine path applies it via optimize/core.py).
    try:
        cap_1min = int(params.get("cap_1min") or 0)
    except (TypeError, ValueError):
        raise ParamError(f"'cap_1min' must be an integer (got {params.get('cap_1min')!r})")
    if cap_1min < 0:
        raise ParamError(f"cap_1min must be ≥ 0 (got {cap_1min}); 0 = time-cap OFF")
    cap_mode = params.get("cap_mode") or "none"
    if cap_mode not in ("none", "bars", "eod"):
        raise ParamError(f"cap_mode must be one of none|bars|eod (got {cap_mode!r})")
    return dict(sl_soft=sl_soft, sl_hard=sl_hard, tp=tp, gate_pct=gate_pct, dd_limit=dd_limit,
                ...
                cap_1min=cap_1min, cap_mode=cap_mode, **split)
...
def build_payload(df4, df1, box, vf, n2025, params=None):
    ...
    sp = SimpleStrategyParams(sl_soft_points=sl_soft, sl_hard_points=sl_hard,
                              ...
                              cap_1min=int(P["cap_1min"]), cap_mode=P["cap_mode"], **_sp_split)
