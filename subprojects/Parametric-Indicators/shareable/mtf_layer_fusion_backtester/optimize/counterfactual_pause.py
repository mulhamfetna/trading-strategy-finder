...
# NOTE (shareable bundle): `from optimize import two_stage as TS` is imported LAZILY inside
# load_champion() — the only function that uses it — so this module (and the causal stack that
# imports only `attribute()`) loads with numpy+pandas alone, without the optuna/optimizer chain.
...
def load_champion(tf: str = "4h"):
    ...
    from optimize import two_stage as TS   # lazy: only this fn needs the optuna/optimizer chain
