...
| [contract sizes + NG stops](./08-TRILINGUAL-contract-sizes-and-NG-stops.md) | #83 #79 | — | why gas's stop can't stop it (a 5.52% weekend gap vs a 0.03% stop), and why futures have no fractions — only a micro/mini **ladder**. Trilingual |
| ⭐ [**architecture audit**](./07-ARCHITECTURE-cli-vs-backend.md) | #91 | — | CLI vs control centre: the backend **already exists** (`main()` is 13% of the file, 24 params ↔ 24 flags). The defect is the *string* interface — preview and execution diverge on **4 of 4** configs. But only **1 of 7** silent failures came from this |
