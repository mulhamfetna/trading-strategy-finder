...
| **WS-SIG-FUSION** | ⏸️ Exogenous signal fusion (VIX/breadth/rates/options) | `dev` | ⏸️ PINNED 2026-07-02 — PARKED, **blocked on team-leader data** (Kalman gate now closed) | orthogonal regime signals → state → policy head (size/SL-TP/sit-out), NOT entry dir; gated by cheap 1-feature pre-test | **UNBLOCK:** team leader provides VIX(+VIX3M) CSV → build loader → 1-feature pre-test. Spec: `docs/EXOGENOUS_SIGNALS_FUSION_WISHLIST.md` |
