...
| **WS-SIG-FUSION** | ⏸️ Exogenous signal fusion (VIX/breadth/rates/options) | `dev` | ⏸️ PARKED — resume after Kalman study closes; **blocked on user data** | orthogonal regime signals → state → policy head (size/SL-TP/sit-out), NOT entry dir; gated by cheap 1-feature pre-test | **RETURN-TO:** `docs/EXOGENOUS_SIGNALS_FUSION_WISHLIST.md`; user provides VIX first |
