...
DST = os.path.join(BASE, "playbooks_backtester_eod1")
...
CAP_TXT = {"eod": "end-of-day exit (never holds overnight)",
           "both": "max-hold {n} bars OR end-of-day — whichever fires first"}

presets = json.load(open(f"{BASE}/presets_raw_eod1.json"))
ver = {(r["inst"], r["tf"]): r for r in json.load(open(f"{BASE}/eod1_verified.json")) if r.get("eod1")}
...
for pr in presets:
    ...
    f, o = m["eod1"]["full"], m["eod1"]["oos2026"]
    ...
    label = (f"{NAME[inst]} {tf} · box + {nind}-indicator gate · {cap} · 1-minute-frame indicators — "
             ...
             f"cold-start re-optimization with the end-of-day close FORCED{warn}")
