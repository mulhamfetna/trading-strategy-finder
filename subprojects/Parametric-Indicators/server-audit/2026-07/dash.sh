...
PORT=8200
# --- WORKSTREAM: timecap-eod (branch: dev) -----------------------------------------------------------
# The dashboard runs from the git WORKTREE, not from a loose rsync'd copy. That is deliberate: what is
# DEPLOYED is now always what is COMMITTED. The old ~/Mulham/wsg-i was an rsync copy with NO git remote,
# sitting on a stale `master` with 61 modified + 509 untracked files -- the precision fix survived there
# only as loose scp'd files, and one `git checkout` would have silently reverted it.
#
#   code   ~/Mulham/code                        branch dev                   <- serves this dashboard
#   code   ~/Mulham/code/.worktrees/fundamental branch fundamental-analysis
#   runs   ~/Mulham/runs/timecap-eod            this workstream's outputs
#   runs   ~/Mulham/runs/fundamental            the FA workstream's outputs
#   data   ~/Mulham/shared/data                 symlinked, never duplicated (33 GB)
CODE=/home/dev/Mulham/code
RUNS=/home/dev/Mulham/runs/timecap-eod
BASE=/home/dev/Mulham/wsg-i          # legacy: still holds pg.env and the data dir the symlink resolves to
APP=$CODE/subprojects/Parametric-Indicators
LOG=$RUNS/logs/dashboard.log
SUP_PID=$RUNS/dash_supervisor.pid
