...
for tf in 2m 5m 15m 1h 2h 4h; do
  for w in 1 2 3 4; do
    setsid python3 optimize/optimizer.py $tf --trials 1425 --folds 5 --study-prefix si1 --instrument SI \
      > ~/Mulham/wsg-i/logs/si1_${tf}_w${w}.log 2>&1 < /dev/null &
  done
done
echo "launched 24 SI workers (6 TF x 4 workers, 1425 trials each = 5700/TF)"
