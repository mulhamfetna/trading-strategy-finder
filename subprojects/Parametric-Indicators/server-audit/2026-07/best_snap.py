...
What this captures, per slot, in one visit:
    presets_raw_best.json  — collectLayer('l1'): the EXACT object the dashboard sends. The shareable
                             backtester replays this. meta.params drops cap_1min/cap_mode/ind_1min/flip,
                             so it cannot be used in its place.
    snapshots_best/*.png   — full-page screenshot of that champion's run, embedded in its playbook PDF.
...
SNAP = os.path.join(BASE, "snapshots_best")
PRESETS = os.path.join(BASE, "presets_raw_best.json")
    ...
    for inst in INSTS:
        for tf in TFS:
            ...
            try:
                ...
                pg.select_option("#champ_set", "best")
                ...
                pg.wait_for_function("() => collectLayer('l1').indicators.length > 0", timeout=30000)
                pg.click("#run")
                pg.wait_for_function(DONE, timeout=2400000)
                d = pg.evaluate(READ)
                c, bx = d["collect"], d["boxes"]
                # the DEPLOYED set is best-per-slot, so cap modes are mixed by design (32 kept champions
                # include none/bars). Only assert that a real champion loaded, not which exit rule it uses.
                if not c.get("indicators"):
                    raise ValueError("no champion loaded — the form is empty")
