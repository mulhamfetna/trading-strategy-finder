...
for f in sorted(CH.glob("*.json")):
    ...
    # NOTE: the sign class MUST escape the hyphen. `[+-−]` is a RANGE from '+' (U+002B) to '−' (U+2212),
    # which swallows the digits — it silently ate the leading digit of every drawdown ($3,990 -> ',990').
    full = num(lab, r"([+\-−]?)\$?([\d,]+) full")
    dd = num(lab, r"\$?([+\-−]?)\$?([\d,]+) DD")
    win = re.search(r"([\d.]+)% win", lab)
    oos = num(lab, r"OOS-2026 ([+\-−])\$([\d,]+)")

    # SELF-CHECK: every parsed number must round-trip back into the label it came from. A regex that
    # silently drops a digit (as the sign-class range bug did) fails here instead of shipping bad numbers.
    for val, unit in ((full, "full"), (dd, "DD")):
        assert val is not None and f"${abs(val):,.0f} {unit}" in lab, \
            f"{f.stem}: parsed {unit}=${(val or 0):,.0f} does not round-trip into label: {lab!r}"
