...
| `probe_xseries_impact.py` | **Issue #75.** Quantifies what the reference-wiring fix CHANGES on real data: (A) the no-reference path must be byte-identical, (CONTROL) `k=99` must move entries or phase B is meaningless, (B) pre-fix vs fixed vs fixed-and-wired entries/PnL, (C) per-trial cost now that they are live. | `python3 -m optimize.perf.probe_xseries_impact --tf 4h --reference ES` |
