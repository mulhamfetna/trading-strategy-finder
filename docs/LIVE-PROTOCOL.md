...
**STATUS: SIGNED 2026-08-31 (owner decision in session; §9).** This file is law: hash-pinned in the ledger
(claim `LIVE-PROTOCOL-SIGNED`); every later change is a dated amendment in §10. Part
...
- **Box feed:** owner-scraped, **on demand** (owner decision 2026-08-31 — no fixed cadence promised). The
  mechanical guard carries the risk instead: if the newest box row is older than 35 calendar days, **no new
  entries** fleet-wide (open positions manage to exit normally); entries resume automatically after a fresh
  drop passes the gate-E merge (`fwd_merge_boxes.py --probe` first — the convention check that caught the ES
  double-shift). Reports state the box frontier date so a paused stretch is visible, never hidden.
...
## 9. Signature
Owner: **Mulham Fetna** · Date: **2026-08-31** · signed in session ("paper only and only … I sign") ·
