...
| ID | use-case | mechanism | design sketch | status |
|---|---|---|---|---|
| **X-4** | **Blindness-hours observability** — annotate the dashboard/causal log with E-D1's schedule | FU-2 proved vetoes don't pay — so this is OBSERVABILITY, not gating: the operator sees which trades happened inside known-blind hours | Ops overlay, byte-identical books, dashboard-only; ship gate = visual parity | **✅ SHIPPED-ON-BRANCH (2026-08-20)**: event_window tags + never-gates meta; response JSON-equal to the pre-captured reference; tagger proven (controls + 65/65); visual gate via ssh-tunnel+Playwright, figures ≡ production, screenshot committed. Claim `X4-BLINDNESS-OBSERVABILITY-SHIPPED`, 64/64 |
| **X-5** | **Monitor × compound power** — do the news regime monitor's drawdown episodes cluster in high-compound-power regimes? | Protective analysis: if yes, the monitor's rolling window could carry a declared context field (information, not a new trigger) | Join the monitor's committed history with X-1's compound-power series; correlation + era split; NO trigger change without a full gate | **✅ INFORMATIVE (2026-08-20)**: ρ +0.906 CI-clear BUT the within-year shuffle bar is 0.879 — the ANNUAL regime carries the bulk (era-2's law re-measured); event-level margin 0.027. X-5b **DELIVERED** (shipped-on-branch: `--context`, trigger untouched proven 3 ways, 29/29 definition parity; claim `X5B-MONITOR-CONTEXT-SHIPPED`). Claim `X5-MONITOR-POWER-INFORMATIVE`, 63/63 |
