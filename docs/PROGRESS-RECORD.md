...
| **RUNG-4 programme opened + #190 backfill done** | #194 | **2026-08-30**: `docs/RUNG4-ROADMAP.md` (4 phases, acceptance criteria; issues #195–#201 + #186 sequenced; rung 5 out of scope). Phase 1 first item CLOSED: the pre-protocol records are ledger-bound — `BEST-SET-SELECTION-RECORD` ($2,202,317.50 stored selection total; composition re-derives 29 incumbent/24 EOD/1 bolt-on; V3 proves silent regeneration would be caught via the corrected-box ES delta), `GAP-FILLS-RISK-TRUTH` (DD +9.78%, P/L −0.17%, mandatory-in-code, causal-not-rescale falsifier), `NOGO-VERDICTS-ON-RECORD` (3 verdicts + no live imports + no schema adoption). Ledger **74/74** |
| **#195 live allowlist FROZEN** | #195 | **2026-08-30**: rule pre-registered before derivation (`docs/LIVE-ALLOWLIST-PREREGISTRATION.md`) → applied to the committed round-2 books → **9/54 slots admitted** (ES 4h/2h/1h/15m · NQ 4h/2h · GC 4h · HG 4h · NG 4h), fresh net@$25 +$37,315 vs $3,844 random-set p95; every criterion proven binding (drop-one falsifiers); `optimize/live/live_allowlist.json` hash-pinned for #199; claim `LIVE-ALLOWLIST-FROZEN`; ledger **75/75** |
| **#196 UI single-engine fix** | #196 | **2026-08-30**: root cause — `/api/backtest_causal` merged CAUSAL money cards with the OLD strategy engine's trades ledger in one response (ES 15m: 180/$14,033 shown vs 205/$28,905 causal; stable, not caching). Fix: the merged L1 payload serves causal trades+equity everywhere; **54/54 counts exact** on the re-run gate, money unchanged, golden ALL MATCH; claim `UI-TRADES-SINGLE-ENGINE`; ledger **76/76**. Plus the #198 hook landed default-off (gate_recal params + `gate_thresholds_recal`, 5 unit tests) |
| **#198 gate-recal cadence: NULL** | #198 | **2026-08-30**: hook off-state = engine (A0 parity **54/54** on real books); quarterly/monthly recalibration reopens the dark slots (NQ 2m 20→46/86 fresh entries) but at $25/rt the fleet is NO better (−$4,070/−$710 vs frozen, CIs straddle 0) and the random-percentile churn control beat both by TRADING LESS (+$36.8k, also n.s.) — **frozen gates ship in #199**. Exploratory (post-hoc, logged only): on the 9 allowlist slots recalibration helped (+$17.1k/+$15.0k) — a pre-registrable #186/#199 hypothesis. Claim `GATECAL-CADENCE-NULL`; ledger **77/77** |
| **#197 ES re-selection: RETAIN ×6** | #197 | **2026-08-31**: fresh campaign on the corrected box (es197b, 5,900 trials/slot in the incumbents' own 18-indicator space after two PRE-RESULTS amendments) beats the incumbents on NO slot under the frozen rule at $25/rt — 2h inside the SE margin, 5m gate-dark, 2m friction-blocked; **the incumbents hold up on the corrected box** ($8k–$17k median-fold). Deployed set + allowlist unchanged; LIVE-PROTOCOL §8 prerequisite met. ⭐ #90 evidence: full-registry cold search 0/2,900 feasible vs 44–80% in the 18-space. Claim `ES197-RETAIN-ALL-SIX`; ledger **78/78** |
| **#199 LIVE-PROTOCOL SIGNED (paper-only)** | #199 | **2026-08-31**: owner decisions in session — PAPER ONLY on the project's own deployment layer; boxes on demand with the 35-day auto-pause guard; `dev` branch protection = the 4 CI checks (main already had them). §9 signed, allowlist hash-bound (40bc5de2b94fa1a4), claim `LIVE-PROTOCOL-SIGNED`; ledger **79/79**. ONE gate left before the track-record clock: the paper executor + shadow-window verification (#213). External executor OUT OF SCOPE (#200 closed) |

## 3 · ACTIVE + QUEUED (the live board)

**The board (2026-08-31): RUNG-4 phases 1–3 signed off — LIVE-PROTOCOL is LAW (paper-only); the single remaining gate is #213 (paper executor + shadow verification), then the track-record clock runs**: no active workstream, no
