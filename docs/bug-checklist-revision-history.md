...
| BUG-027 | Trade log displayed prices that don't exist in the dataset — `avg_entry_price` (synthetic blend of leg fills) and `exit_price` (SL/TP threshold line) shown as the primary Entry / Exit prices; users couldn't verify trades against the chart | UX / data semantics | High | user-2026-05-24 | **FIXED 2026-05-24** in commit `8cc5afb` — trade dict gains `entry_signal_price` (= `legs[0].price`, signal-bar close, always in OHLC) and `exit_close` (= bar close at exit, always in OHLC). Frontend renders these as the primary columns; tooltip + CSV export surface the algorithm-effective prices. PnL math unchanged. Regression tests in `tests/test_trade_log_alignment.py` (3 cases). | Display fields must be candle-grounded; algorithm-effective values move to tooltip / second column |
| BUG-028 | Soft SL fill was line-based, not close-based — contradicting the user-spec "soft SL loss is the closing price of the candle, not the soft-sl value". Loss was capped at `sl_soft_points` even when the bar closed dramatically further past the line | Strategy / risk model | High | user-2026-05-24 | **FIXED 2026-05-24** in commit `6b9ba4d` — `_check_exits` returns `exit_price=close` for SOFT (both long and short branches). HARD SL keeps line fill (loss = exactly `sl_hard_points` — disaster stop contract). Documented in MASTER_STRATEGY_GUIDE §4 as the asymmetric fill rule. Regression test: `tests/test_trade_log_alignment.py::test_soft_sl_fills_at_bar_close_not_at_line`. | Soft tier fills at confirming bar close; hard tier fills at the line |
| BUG-029 | `frontend/src/stores/backtest.ts:75` TS narrowing error — `.message` accessed on `{detail: string}` type | Technical contract | Low | discovered 2026-05-24 | **OPEN** — pre-existing before this session, surfaced when re-running `npm run build`. Code reads `ev.data.detail ?? ev.data.message ?? 'Unknown error'` but the SSE error event's TS type only declares `detail`. Fix: widen the error event union to `{detail: string} | {code, message, system_status}`. | Widen the SSE error type union |
| BUG-030 | `load_data` skipped Date-column parsing on pandas 3 — `dtype == object` check is false when pandas returns `str` dtype, leaving Date as raw strings. Date arithmetic (`walk_forward.split_folds`) crashed with `TypeError: unsupported operand type(s) for -: 'str' and 'str'` | Data pipeline | High | discovered 2026-05-24 | **FIXED 2026-05-24** in commit `283917e` — `src/data/loader.py` now gates on `not pd.api.types.is_datetime64_any_dtype(...)` instead of `dtype == object`. Affects every code path that does Date arithmetic (walk-forward folds, sub-bar slicing). Discovered while wiring NSGA-II H.1 SSE tests. | Use the typed dtype check, not equality with `object` |
| BUG-031 | Trade-log entry/exit prices rendered as `NaN` after a backtest — root cause: backend serving pre-#118b code while frontend was post-#118d. The `entry_signal_price` / `exit_close` fields weren't in the SSE payload, frontend Pinia store left them undefined, `Intl.NumberFormat` returned "NaN" | Operational / dev workflow | Medium | user-2026-05-24 (saved dashboard HTML 12:01) | **CONFIGURATION** — not a code bug per se; `uvicorn --reload` watches Python files but not always reliably. Mitigation: documented in `AGENTS.md` "Restart `uvicorn` after Python edits". No regression test possible. | Restart the backend after any Python-side change; if you see `NaN` in the dashboard, suspect a stale backend process first |
