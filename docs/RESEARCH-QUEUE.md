...
|---|---|---|---|---|
| RQ-1 | **ES CPI scaling study** (D3/D4-style: per-second participation, worked entry, qty 5/10/20 for the ES CPI ride) | WS-ESCPI blind spot — qty=1 arithmetic only; ES has the deepest book of the four indexes | #141 | QUEUED (contingent on #139 shipping) |
...
| RQ-3 | **ES CPI forward paper-confirmation track** — paper intents beside the live layer; future CPI prints vote (~6–12 months to power) | the #139 ship-decision option 2; design pre-drafted so it can start the day the owner picks it | #143 | QUEUED (owner decision #139) |
...
| RQ-7 | **YM CPI promotion path** — the grid's only positive (+$107.64 net, p=0.0016, jump 9.8×; thin tape median 101 pre-release seconds ⇒ execution study required, not just statistics) | WS-GRID + the ESCPI descriptive; same data as #139's evidence — rides on the same owner decision | #147 | QUEUED (owner decision #139) |
