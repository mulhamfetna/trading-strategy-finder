...
| **FU-13** (#165) | **Exp2 size-WITH-vol ramp — end-to-end verification & deployment** (owner-ordered 2026-08-19) | The parked winner: Ret/DD 5.52→5.90, beats 95% of random ramps, all 3 years; a dashboard overlay already exists OFF-default | reproduce → promotion battery (longer book/OOS ramp/DD cap) → core (golden-locked default-off) → dashboard+screenshots → ledger → deploy+playbook | **ACTIVE** |
| **FU-14** (#166) | **M2 power model productionization** (owner-ordered 2026-08-19) | Confirmed forecast (ρ≈0.5 night-before), consumed by NO live layer — ship the ENGINE (nightly forward forecasts, parity-bound), consumers stay separate gated studies | build src/deploy/power_forecast.py on the committed M2 methodology → parity/falsifier battery → ops artifact → ledger → deploy+playbook | **ACTIVE** |
