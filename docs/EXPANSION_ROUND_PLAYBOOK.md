...
| **C19** | **Decide whether a feature is ACTIVE from the object that produces its output, never from the configuration that was requested.** | #75: `confirm_mask` asked "was a reference configured?" while the votes came from a context that had none. The cross-series indicator was counted among the confirmers — raising `k_eff = min(k, len(confirmers))` — while contributing zero confirmations, so enabling one took the strategy from **13 entries to 0**. Reading the vote-producing context instead makes the two agree by construction: forget to thread the reference at a new call site and the indicator goes *inert*, which is safe, instead of becoming a confirmer that can never confirm. |
| **C20** | **Before trusting "no impact", make the measurement prove it CAN see an impact.** | #75: the first impact run used `k = champion_k + 1 = 2` and reported a difference of exactly zero for all four indicators. The champion already carries 7 confirmers, so `k=2` was satisfied whatever the new one did — the comparison was **unfalsifiable, not null**. A `k=99` control (277 → 13 entries) exposed it, and re-running with `k = n_confirmers + 1` turned "+0" into "**0 entries vs 13**". |
| **C21** | **A feature with a flag, a schema entry and a place in the search space has not necessarily ever RUN.** Execute it end-to-end once before believing any measurement about it. | #75: four cross-series indicators had a CLI flag, docs, tests and search-space membership — and three stacked defects meant none had ever executed on the production path. One of them (`build_entry_resolver`) raised `NameError` on first contact, invisible because `and` short-circuits for every indicator with `needs_ref=False`. |
